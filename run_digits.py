...
OP_DIRECTORY = os.path.join(os.getcwd(), 'output')
...
def plotfigs2(xx, selectedy, fileprefix=None, printselectionnumbers=False):
    ...
    for ii in range(10):
        num_selected[ii] = len(np.where(selectedy == (ii))[0])
    ...
    while counter < int(totm / perpic_m) + 1:
        ...
        # print(int(int(perpic_m / perrow) + offset / perrow))
        ...
        for i in np.array(range(begin_at + offset, endd)):
            ...
            ax.imshow(xx[i - offset, :].reshape((28, 28)))
            ax.axis('off')
...
def main(
    ...
    selected, critselected, digitsdat = run(
        os.path.join(data_prefix, 'mnist_data'),
        gamma,
        m,
        k,
        kerneltype,
        outfig,
        critoutfig,
        os.path.join(data_prefix, 'mnist_data.t'))
...
if __name__ == '__main__':
    ...
    # global M, e
    # plt.plot(M, e, color='green', marker='o', linestyle='dashed', linewidth=2, markersize=12)
    # plt.show()
